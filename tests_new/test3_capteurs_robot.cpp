...
int main(int argc, char* args[])
{
	...
	vector <float> positions_capteurs_cm = { 4,6.0,8.0,12.5 };
	MapRobot map(graphics, "../assets/robot1.tmx", map_shapes.getLignes(), map_shapes.getArcs());
	Rectangle barre = map.getBarre();

	SDL_Point size = map.getSize();
	Fps fps(graphics);
	graphics.resize(size.x, size.y);
	map.printData();
	Point center=map.getRobotCenter();
	Direction direction = map.getRobotDirection();
	Robot robot(map,map_shapes.getImageTexture(),center,direction, positions_capteurs_cm);
	...
	vector<float>capteurs;
    bool running = true;
    //While application is running
	int angle = 0;
	int cpt=0;
	bool sens_l = true, sens_r = true;
    while( running )
    {
        input.beginNewFrame();
        ...
        else if (input.wasKeyPressed(SDLK_q))
        {
            running = false;
        }
		...
		if (cpt %50==0)
		{
			sens_l = !sens_l;
			sens_r = !sens_r;
			robot.sens(sens_l, sens_r);
		}*/
		int value=barre.collidesWith(robot.getRectangle());

		coins.update();		
		fps.update();
		fps.draw(0,size.y-20);
		Text text_time(graphics, tim.getTime().c_str(), font, SDL_Color{ 200,50,50 });
		text_time.draw(graphics, size.y-50, 10);
		map.draw(graphics,false);
		coins.draw(graphics);

		robot.draw(graphics);
		capteurs = robot.getCapteurs();
		cout << "value=" << value << "  ";
